...
fieldweather %>% 
  ...
  ggplot()+
  geom_density(aes(value, fill = name, alpha = 0.4))+
...
fe_month_sum <- fe_month %>% 
  group_by(site, month_abb, year, strength_gtex) %>% 
  summarise(across(.cols = c(dayl.s, srad.wm2, tmax.c, tmin.c, vp.pa), 
                   .fns = mean, 
                   .names = "mean_{.col}"),
            across(.cols = prcp.mm,
                   .fns = sum,
...
fe_month_sum_wide <- fe_month_sum %>% 
  pivot_longer(5:10) %>% 
  mutate(name = paste0(name,"_",month_abb)) %>% 
...
fe_month_sum %>% 
  ggplot(aes(x = mean_tmax.c, y = month_abb, fill = stat(x)))+
  geom_density_ridges_gradient(scale = 1.5, rel_min_height = 0.01)+
  scale_fill_viridis_c(option = "C", alpha = 0.6)+
  ...
  mutate(plot = map2(data, name,
                     ~ggplot(data = .x, aes(value, 
                       ...
                       geom_density_ridges_gradient(scale = 2,
                                                    rel_min_height = 0.01)+
                       scale_fill_viridis_c(option = "C", alpha = 0.6)+
                       labs(x = paste("Monthly-averaged", .y),
                            y = "Month")+
                       theme(legend.position = "none")))
